...
const logInUserSignInSchema = z.object({
  email: z
    .string()
    .email({ message: 'Coloque um email' }),
  password: z
    .string()
...
type TUserSignInSchema = z.infer<typeof logInUserSignInSchema>

export default function SignIn() {

  const router = useRouter();
  const logInUserSignIn = useForm<TUserSignInSchema>({
    resolver: zodResolver(logInUserSignInSchema)
  })
  ...
  function logInUsers(data: TUserSignInSchema) {
    console.log(data)
    router.push('/');
  }
  ...
  return (
    <section className="bg-gradient-to-b from-gray-100 to-white">
      <FormProvider {...logInUserSignIn}>
        <div className="max-w-6xl mx-auto px-4 sm:px-6">
          <div className="pt-32 pb-12 md:pt-40 md:pb-20">
            <div className="max-w-3xl mx-auto text-center pb-12 md:pb-20">
              <h1 className="text-4xl font-extrabold leading-tight tracking-tighter">Bem vindo de volta!</h1>
            </div>
            <div className="max-w-sm mx-auto">
              <form onSubmit={handleSubmit(logInUsers)}>
                ...
                <div className="flex flex-wrap -mx-3 mb-4">
                  <div className="w-full px-3">
                    <div className="flex justify-between">
                      <label className="block text-gray-800 text-sm font-medium mb-1" htmlFor="password">Senha</label>
                      <Link href="/reset-password" className="text-sm font-medium text-blue-600 hover:underline">Esqueceu a senha?</Link>
                    </div>
                  ...
                  <div className="w-full px-3">
                    <div className="flex justify-between">
                      <label className="flex items-center">
                        <input type="checkbox" className="text-gray-800 rounded-sm bg-white border border-gray-300 focus:border-gray-500" />
                        <span className="text-gray-600 ml-2">Mantenha-me conectado</span>
                      </label>
                    </div>
                  </div>
                </div>
                <div className="flex flex-wrap -mx-3 mt-6">
                  <div className="w-full px-3">
                    <button className="px-8 py-3 shadow-lg text-white bg-blue-600 hover:bg-blue-700 w-full">Entrar</button>
                  </div>
                </div>
              </form>
              ...
              <div className="text-gray-600 text-center mt-6">
                Não tem conta? <Link href="/signup" className="text-blue-600 hover:underline transition duration-150 ease-in-out">Criar conta</Link>
              </div>
            </div>
